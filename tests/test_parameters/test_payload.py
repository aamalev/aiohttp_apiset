...
async def test_payload_reader(aiohttp_client):
    ...
    assert len([k for k, v in reader]) == 4

    reader['*/*'] = any_reader
    assert len(reader) == 5
    assert '*/*' in reader
    assert reader['*/*'] is any_reader
    del reader['*/*']
    ...
    async def handler(request):
        ...
        try:
            ...
            return web.json_response(exc.to_tree())
        except ValueError as exc:
            return web.json_response({'msg': str(exc)})
        else:
    ...
    assert rep.status == 200, await rep.text()
    data_type = (await rep.json())['data_type']
    assert data_type == 'MultiDictProxy'
    ...
    assert rep.status == 200, await rep.text()
    msg = (await rep.json())['msg']
    assert msg == 'Unsupported content type'
